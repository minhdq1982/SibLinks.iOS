...
private let cellIdentifier = "QueryViewCell"

class SLQueryViewController: SLBaseViewController, UITableViewDataSource, UITableViewDelegate, UIScrollViewDelegate {
    
    @IBOutlet weak var tableView: UITableView!
    /**
     Refresh control added to table view
     */
...
//    var loading = false
    ...
    override func loadView() {
        ...
        if pullToRefreshEnabled {
            ...
            refreshControl.addTarget(self, action: #selector(SLQueryViewController.refreshControlValueChanged(_:)), forControlEvents: .ValueChanged)
    ...
    // MARK: - Remove object
    func removeObject(object: DataModel) {
        if mutableObjects.contains(object) {
...
//            self.tableView.reloadRowsAtIndexPaths([indexPathForPaginationCell()], withRowAnimation: .None)
    ...
    // MARK: - Loading View
    func refreshLoadingView(error: NSError?) {
...
extension SLQueryViewController {
    // MARK: - Empty State
