...
@objc protocol SideMenuControllerDelegate {
    optional func sideMenu(sideMenu: SideMenuController, didRecognizePanGesture recognizer: UIPanGestureRecognizer)
    optional func sideMenu(sideMenu: SideMenuController, willShowMenuViewController menuViewController: UIViewController)
    optional func sideMenu(sideMenu: SideMenuController, didShowMenuViewController menuViewController: UIViewController)
    optional func sideMenu(sideMenu: SideMenuController, willHideMenuViewController menuViewController: UIViewController)
    optional func sideMenu(sideMenu: SideMenuController, didHideMenuViewController menuViewController: UIViewController)
}

@IBDesignable class SideMenuController : SLBaseViewController, UIGestureRecognizerDelegate {
    ...
    @IBInspectable let contentViewStoryboardID: String? = nil
    @IBInspectable let leftMenuViewStoryboardID: String? = nil
    @IBInspectable let rightMenuViewStoryboardID: String? = nil
    @IBInspectable var panFromEdgeZoneWidth : CGFloat = 20.0
    @IBInspectable var panGestureLeftEnabled : Bool = false
    @IBInspectable var panGestureRightEnabled : Bool = false
    @IBInspectable var interactivePopGestureRecognizerEnabled: Bool = true
    @IBInspectable var scaleContentView: Bool = true
    @IBInspectable var fadeMenuView: Bool = true
    @IBInspectable var backgroundTransformScale: CGFloat = 1.7
    @IBInspectable var scaleBackgroundImageView: Bool = true
    @IBInspectable var scaleMenuView: Bool = true
    @IBInspectable var contentViewShadowEnabled: Bool = false
    @IBInspectable var contentViewShadowColor: UIColor?
    @IBInspectable var contentViewShadowOffset: CGSize = CGSizeZero
    @IBInspectable var contentViewShadowOpacity: Float = 0.4
    @IBInspectable var contentViewShadowRadius: CGFloat = 8.0
    @IBInspectable var contentViewFadeOutAlpha: CGFloat = 1.0
    @IBInspectable var contentViewScaleValue: CGFloat = 0.85
    @IBInspectable var contentViewInLandscapeOffsetCenterX : CGFloat = 30.0
    @IBInspectable var contentViewInPortraitOffsetCenterX: CGFloat = 30.0
    @IBInspectable var parallaxMenuMinimumRelativeValue: CGFloat = -15
    @IBInspectable var parallaxMenuMaximumRelativeValue: CGFloat = 15
    @IBInspectable var parallaxContentMinimumRelativeValue: CGFloat = -25
    @IBInspectable var parallaxContentMaximumRelativeValue: CGFloat = 25
    ...
    var panFromEdge: Bool = true
    var panMinimumOpenThreshold: Float = 60.0
    var menuPreferredStatusBarStyle: UIStatusBarStyle = UIStatusBarStyle.Default
    ...
    override func viewDidLoad() {
        ...
        self.view.backgroundColor = UIColor(hexString: Constants.SIBLINKS_SIDE_MENU_BACK_COLOR)
        ...
        button.addTarget(self, action: #selector(SideMenuController.hideMenuViewController), forControlEvents: UIControlEvents.TouchUpInside)
        ...
        if (self.panGestureEnabled) {
            ...
            let panGestureRecognizer: UIPanGestureRecognizer = UIPanGestureRecognizer(target: self, action: #selector(SideMenuController.panGestureRecognized(_:)))
    ...
    func showLeftMenuViewController() {
        ...
        self.contentViewContainer.layer.cornerRadius = 10
        self.contentViewContainer.clipsToBounds = true
        UIView.animateWithDuration(self.animationDuration, animations: {
...
//        UIApplication.sharedApplication().setStatusBarStyle(.Default, animated: true)
        //(2,Mar, 2017) Jimmy's comments: Change color staus bar to Light
        UIApplication.sharedApplication().setStatusBarStyle(.LightContent, animated: true)
        ...
        let completionBlock = { [weak self] in
            self!.contentViewContainer.layer.cornerRadius = 0
